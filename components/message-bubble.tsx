...
export function MessageBubble({ message, isCurrentUser }: MessageBubbleProps) {
  const formattedTime = formatDistanceToNow(new Date(message.sent_at), { addSuffix: true })
  ...
  return (
    <div className={cn("flex mb-4", isCurrentUser ? "justify-end" : "justify-start")}>
      ...
      <div className={cn("max-w-[70%]")}>
        {!isCurrentUser && (
          <div className="text-xs text-gray-400 mb-1 ml-1">{message.display_name || message.username}</div>
        )}
        ...
        <div className={cn("text-xs text-gray-400 mt-1", isCurrentUser ? "text-right mr-1" : "ml-1")}>
          {formattedTime}
        </div>
      </div>
    </div>
